defmodule RobotSimulatorTest do
  ...
  # @tag :pending
  test "moves left for any position" do
    robot1 = RobotSimulator.create(:north, {0, 0}) |> RobotSimulator.simulate("L")
    assert RobotSimulator.direction(robot1) == :west

    robot2 = RobotSimulator.create(:east, {0, 0}) |> RobotSimulator.simulate("L")
    assert RobotSimulator.direction(robot2) == :north

    robot3 = RobotSimulator.create(:south, {0, 0}) |> RobotSimulator.simulate("L")
    assert RobotSimulator.direction(robot3) == :east

    robot4 = RobotSimulator.create(:west, {0, 0}) |> RobotSimulator.simulate("L")
    assert RobotSimulator.direction(robot4) == :south
  end

  # @tag :pending
  test "moves right for any position" do
