defmodule ScaleGenerator do
  ...
  @chromatic_scale ~w(A A# B C C# D D# E F F# G G# A A# B C C# D D# E F F# G G#)
  @flat_chromatic_scale ~w(A Bb B C Db D Eb E F Gb G Ab A Bb B C Db D Eb E F Gb G Ab)
  ...
  def chromatic_scale(tonic \\ "C") do
    do_chromatic_scale(tonic, @chromatic_scale)
  end
  ...
  def flat_chromatic_scale(tonic \\ "C") do
    do_chromatic_scale(tonic, @flat_chromatic_scale)
  end
  ...
  def scale(tonic, pattern) when tonic in ~w(F Bb Eb Ab Db Gb d g c f bb eb) do
    do_scale(tonic, pattern, @flat_chromatic_scale)
  end
  ...
  defp do_scale(tonic, pattern, ref_scale) do
    tonic = tonic |> String.capitalize()

    pattern
    |> String.graphemes()
    |> Enum.reduce([tonic], fn step, [note | _] = scale ->
      [step(ref_scale, note, step) | scale]
    end)
    |> Enum.reverse()
  end

  defp do_chromatic_scale(tonic, ref_scale) do
    tonic = tonic |> String.capitalize()
